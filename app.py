...
###############################################################################
def main():
    # ---------------  QUICK PRINT LINK (browser native) -------------
    st.markdown(
        """
    ...
    if st.button("Download full page as PDF"):
        with st.spinner("Building PDF…"):
            # 1) grab the current page HTML (Streamlit provides it via components.html)
            #    NB:  we escape it so the <script> that follows is valid.
            html_source = components.html(
                f"""
                <script>
                    // send the entire outerHTML back via postMessage
                    const htmlSource = btoa(unescape(encodeURIComponent(document.documentElement.outerHTML)));
                    parent.postMessage({{type:"htmlSource", data:htmlSource}}, "*");
                </script>
                """,
                height=0, width=0
            )

        # 2) wait for the postMessage
        html_code_holder = st.empty()
        html_code = st.runtime.scriptrunner.script_run_context.get_script_run_ctx().session_state.pop("htmlSource", None)
        if html_code is None:
            st.error("PDF generation failed – reload the page and try again.")
        else:
            # 3) decode & write PDF
            full_html = base64.b64decode(html_code).decode("utf-8")
            ...
            st.download_button(
                ...
                data=pdf_bytes,
                file_name="SurveyDesign_Page.pdf",
